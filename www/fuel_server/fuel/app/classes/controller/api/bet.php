...
class Controller_Api_Bet extends Controller_Apibase
{
    ...
    public function get_send()
    {
        ...
        if ($periodList == null) {
            return $this->response(array('code' => '5', 'message' => Lang::get('error.ER6')));
        }
    ...
    public function get_result()
    {
        ...
        $periodList = $this->redis->get($pid);
        if ($periodList == null) {
            return $this->response(array('code' => 1, 'message' => Lang::get('error.ER6')));
        }
        $period = json_decode($periodList);
        $this_round = Model_Round::find_by_open($period->pid_);
        if ($this_round == null) {
            return $this->response(array('code' => 2, 'message' => Lang::get('error.ER9')));
        }
